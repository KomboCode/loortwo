...
const LoanSchedule = ({ person}) => {
    ...
    const { release, duration, number, principal, interest, 
        repayments, cycle, interestPer } = person;

    console.log("pers", person);

    const [ cpr, setCpr ] = useState(parseInt(principal)); 
    ...
    const interestPerday =  interestPer * principal/100*1/number;
    const interestNum =  interestPer * principal/100;

    const pureInterest =  interest * principal/100;
    ...
    var compp = parseInt(principal);
    var compi = compp * interest/100;
    ...
    for ( var m = 1; m <= number; m++ ) {
        ...
        if ( m === 1 ) {
            compoundDue = compp + compi;
        }


        if ( m !== 1 ) {
            compp = compp + compi;
            compi = compp * interest/100;

             compoundDue = compp + compi; 
        }

        console.log("tione ssada compound principal", compp);
        console.log("gdsfg cound interest", compi);
    ...
    return (
        <section className='loan-schedule'>
           ...
           <div className='info'>
            ...
            kumitatu, kuminne, compoundP, compoundI } = item;
                ...
                <div className='payment'>
                    <div  
                      ...
                      >
                        ...
                        <h4>{Intl.NumberFormat("en-US",options).format(compoundP)}.00</h4>
                        }
                        { tano === "Interest" ? <h4>{tano}</h4> : 
                        <h4>{Intl.NumberFormat("en-US",options).format(compoundI)}.00</h4>
                        }
                        ...
                        { zero === "Due" ? <h4>{zero}</h4> : 
                        <h4>{Intl.NumberFormat("en-US",options).format(compoundP + compoundI)}.00</h4>
                        }
