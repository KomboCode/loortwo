...
const LoanSchedule = ({ person}) => {
    ...
    const { release, number, principal, interest, 
        repayments, cycle, tinterest, method } = person;
    ...
    const interestPerday =  tinterest/number;
    const interestNum =  tinterest;
    ...
    var due = interestNum + parseInt(principal);
    ...
    var kila = due/number;
    if ( method === "Reducing Balance-Equal Installments" ) {
        kila = payable;
    }
    // var maokoto = [];
    // var makopa = [];
    ...
    for ( var m = 1; m <= number; m++ ) {
        ...
        if ( m !== 1 ) {
            ...
            reducePrincipal = payable - reduceInterest;
        } else if ( m === 1) {
            reduceInterest = parseInt(principal) * rate;
            reducePrincipal = payable - newInterest;
        }

            prinDue = prinDue + reducePrincipal;

            console.log("reducing princeipal", prinDue);



        // if ( m !== 1 ) {
        //     reduceInterest = remain * rate;
        //     amountDue = parseInt(principal) + reduceInterest;
        //     remain = amountDue - payable;
        //     console.log("Rateeeeeeeeeeeeee", amountDue);
        // }
    ...
    const divider = rsum/kila;
    const paidPrinc = divider * principalPerday;

    console.log("principal perday", divider);
    ...
    return (
        <section className='loan-schedule'>
           ...
           <div className='info'>
            ...
            kumitatu, kuminne, payable, reduceInterest, reducePrincipal, principalDue } = item;


            
            var wekaa =  <h4>{Intl.NumberFormat("en-US",options).format(zero)}.00</h4>

            
            if ( method === "Reducing Balance-Equal Installments" ) {

                    wekaa =  <h4>{Intl.NumberFormat("en-US",options).format(payable)}.00</h4>
                }

            var reint =  <h4>{Intl.NumberFormat("en-US",options).format(tano)}.00</h4>

            if ( method === "Reducing Balance-Equal Installments" ) {
                reint =  <h4>{Intl.NumberFormat("en-US",options).format(reduceInterest)}.00</h4>
        
            }

            var reprin = <h4>{Intl.NumberFormat("en-US",options).format(nne)}.00</h4>

            if ( method === "Reducing Balance-Equal Installments" ) {
                reprin = <h4>{Intl.NumberFormat("en-US",options).format(reducePrincipal)}.00</h4>
        
            }

            var pdue = <h4>{Intl.NumberFormat("en-US",options).format(kumimoja)}.00</h4>

            if ( method === "Reducing Balance-Equal Installments" ) {
                pdue = <h4>{Intl.NumberFormat("en-US",options).format(principalDue)}.00</h4>
            }
            ...
            return (
                <div className='payment'>
                    ...
                    <div className='other below'
                       ...
                       <h4 className='mtatu'>
                       {Intl.NumberFormat("en-US",options).format(tinterest)}.00
                       </h4>
                    ...
                    <div className='other below'
                       ...
                       <h4 className='mtatu'>
                       {Intl.NumberFormat("en-US",options).format(paidPrinc)}.00
                       </h4>
                       
                       <h4 className='mtatu'>
                       {Intl.NumberFormat("en-US",options).format(rsum - paidPrinc)}.00
                       </h4>
                       <h4 className='mtano'>0</h4>
                       <h4 className='msita'>0</h4>
                    </div>
                    ...
                    <div className='other below mwisho'
                       ...
                       <h4 className='mtatu'>
                       {Intl.NumberFormat("en-US",options).format(principal - paidPrinc)}.10
                       </h4>

                       
                       <h4 className='mtatu'>
                       {Intl.NumberFormat("en-US",options).format(tinterest - (rsum - paidPrinc))}.078
                       </h4>
                       <h4 className='mtano'>0</h4>
                       <h4 className='msita'>0</h4>
                    </div>
